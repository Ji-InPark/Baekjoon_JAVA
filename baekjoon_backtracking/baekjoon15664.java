...
public class baekjoon15664 {
	...
	public static void main(String[] args) throws IOException {
		...
		solve(arr, M, 0, select, 0, result);
		bw.flush();
	}
	
	public static void solve(int[] arr, int M, int cnt, boolean[] select, int pre, int[] result) throws IOException
	{
		if(M == cnt)
		{
			for(int i = 0; i < M; i++)
		...
		else
		{
			for(int i = pre; i < arr.length; i++)
			{
				if(!select[i])
				{
					...
					{
						result[cnt] = arr[i];
						solve(arr, M, cnt + 1, select, i + 1, result);
					}
					select[i] = false;
				}
			}
